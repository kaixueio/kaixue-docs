...
这里变量 `text` 中支持使用 `$` 符号插入变量，也可以进行换行操作，且末尾的 `\n` 转义字符不会生效，这就是「原生字符串」。输出结果如下：
...
- `forEach`：遍历每一个元素
  ...
  ```kotlin
  🏝️
  //              👇 lambda 表达式，i 表示传进来的 Int 类型参数，-> 后接函数体
  intArray.forEach { i ->
...
在 Java 语言中并没有 `Range` 的概念，Kotlin 中的 `Range` 表示区间的意思。区间的常见写法如下：
...
这里的 `0..1000` 就表示一个 `Int` 类型的闭区间 [0,1000]，除了 `Int` 类型的 `IntRange` 还有 `CharRange` 以及 `LongRange`。
