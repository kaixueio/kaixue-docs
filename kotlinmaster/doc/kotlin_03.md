...
由于 `init` 代码块可以看成是主构造器的一部分//去掉，因此当使用次构造器创建对象时，初始化代码块是先于次构造器执行的。//如果把主构造器看成人体的头部，init 就是颈部，次构造器就相当于身体其他部分

细心的你也许会发现这里又出现了 `:` 符号，它在其他场合也出现过，例如：
...
但有些场景，`constructor` 是不可以省略的，例如主构造器需要使用注解或者可见性修饰符：
...
看到这里，已经非常简单了，还会不会更简单呢？

我们会发现一开始那段五六行的 `User` 类，已经简化到还有三行了。那么，还有没有更简单的写法呢？下面我们来看看，如何在主构造器中声明属性，把上面这段三行的代码简化到只剩一行！

#### 主构造器里声明属性

之前我们讲了 name 参数，还有更简单写法

上一小节我们了解了主构造器，其实在 Kotlin 中，还可以将主构造器中的参数直接作为该类的属性：
...
以上讲了所有关于主构造器相关的知识，让我们总结一下一个类的初始化写法：// TODO
...
这是一个非常简单的函数，获取到参数 `name` 值并输出。在 Kotlin 中，这种只有一行代码的函数，还可以这么写：// 先讲返回值，再这句
...
其实在  Java 中，每个重载函数的内部实现可以各不相同，这就无法保证重载方法内部设计的一致性。在调用这些重载方法时，每一个都能有相同的执行效果。而 Kotlin 的参数默认值，就解决了这个问题。

以上就是函数默认参数值的概念。如果函数中，一个有默认值的参数在一个无默认值参数的前面，这时就无法调用
...
这点 kotlin 在设计上也考虑到了，使用命名参数来解决这个问题。
...
这个函数中一些参数有默认值，一些参数又没有默认值，此时我们调用这个函数会是这样：
...
当看到后面一长串的布尔值时，很难分清楚每个参数的用处，可读性很差。好在 Kotlin 为我们提供了命名参数，使用命名参数我们可以这样调用：
...
在调用函数时，我们显式地指定了每个参数的名称，这就是命名参数。Kotlin 中的每一个函数都可以具备命名参数的特性。// 中文引号 着重

与命名参数相对的一个概念被称为位置参数，也就是按位置顺序进行参数填写。当一个函数被调用时，如果混用位置函数与命名参数，那么所有的位置参数都应该放在第一个命名参数之前。我们看下面一个例子：
...
sayHi("wo", age = 21) // 左指  位置参数 name 在命名参数 age 之前，这样是正确的
      
sayHi(name = "wo", 21) // 命名参数 name 后面出现了位置参数，这是不允许的 报错
```
...
讲了这么多函数名里方便的写法，那函数体里有没有什么方便的使用方法呢？接下来，我们再看看 Kotlin 中的另一种常见函数：嵌套函数。// 生硬  讲完了
...
可以发现函数中检查参数这个部分有些冗余，你 又不想将这段逻辑作为一个单独的函数对外暴露// todo，但这些重复逻辑看着又实在难受。这时你可以使用嵌套函数，在 `login` 函数内部声明一个函数：
...
上面这段代码看起来比上面简洁多了，我们将共同的验证逻辑放进了嵌套函数  `validate` 中，并且 `login` 函数之外的其他地方是访问不了这个嵌套函数的。
...
这里省去了嵌套函数中的 `illegalStr` 参数，在该函数内直接使用外层函数 `login` 的参数 `illegalStr`，这就不用再声明参数。// 陈述 简洁少用
...
// 其中用到 lambda 表达式以及 kotlin 提供给我们的 require 函数，这里就不再展开了，之后会讲。

Kotlin 还为我们提供了许多类似 `require` 这样方便的函数，我们会在之后的文章中继续讲到。

至此//少用，我们知道了函数的 `= ` 连接符、参数默认值、命名参数、嵌套参数，这些知识点都可以将平时写的函数进行更方便的简化使用。再回过头来看看这段代码：
...
其中的 `|` 符号为默认的边界前缀，表示这一行 `|` 符号之前的所有空格需要去除。也可以选择其他字符做边界前缀，比如 `trimMargin("/")`。输出结果如下：
