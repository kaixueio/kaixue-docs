...
在上期课程当中，我们知道了 Kotlin 的那些与 Java 写法不同的地方。这一期我们再进阶一点，讲一讲 Kotlin 中那些「更方便的」用法。这些知识点在不知道之前，你也可以正常写 Kotlin，但是在熟悉之后能够让你写得更爽。
...
这个写法我们叫「主构造器 primary constructor」。与之相对的在第二篇中，我们写在类中的构造函数被称为「次构造函数」。在 Kotlin 中最多只有 1 个主构造器（可以没有），以及 0 到 N 个次构造器。// todo 次构造器
...
其中 `init` 代码块是紧跟在主构造器之后执行的，可以在其中写一些初始化逻辑，这就相当于主构造器的代码块。// 说明主构造器没有代码块

// 显示声明主构造 有无参数， 另外，如果类中有一个主构造器，那么其他次构造函数都需要通过 `this` 关键字调用主构造器，可以直接调用或者通过别的次构造函数间接调用。如果不调用会怎样呢：
...
上面的代码没有通过 `this` 关键字调用主构造器，就会报错提示需要调用主构造器。为什么次构造器一定要主构造器呢？因为对象的初始化过程必须要有主构造器的参与，如果次构造函数没有委托给主构造器，在初始化过程中是缺少参数的，也就是非法的。// 当你声明一个主构造器的时候，主构造器就是必须性，2.第一性 首先调用，这也就是称之为主构造器的原因，
...
可以发现，我们通过 `this` 关键字将次构造函数中的 `name` 作为参数调用了主构造器。如果这时候类中还有 `init` 初始化代码块的话，由于初始化代码块实际也是主构造器的一部分，所以初始化代码块是在次构造函数之前执行的。// :  用法，继承 ，变量声明，依赖关系

除此之外，主构造器也可以有注解或者可见性修饰符，都是放在 `constructor` 前面，如果不需要注解或可见性还可以省略 `constructor` 关键字：// 大部分情况下 c 可以省略，某些情况下
...
上一小节我们了解了主构造函数的概念。其实在 Kotlin 中，还支持将主构造器中的参数直接作为该类的属性：
...
// 到这里，整个类就完成了，并且我们整个类的初始化顺序也是从上到下

这时一个有着两个构造函数，以及多个属性的 `User` 类仅仅几行就写完啦，是不是非常方便。以上就是对主构造器所有概念的回顾，并且最终的调用执行顺序也与刚才我们书写的顺序相同。

至此，我们对构造函数中方便的写法有了一定了解，其实普通函数也有许多更简单的写法，下面让我们来看看普通函数的那些「更方便的」。// 语病，读者的角度  细节
