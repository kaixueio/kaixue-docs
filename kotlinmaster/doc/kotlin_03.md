...
在使用次构造器创建对象时，`init` 代码块是先于次构造器执行的。我们可以做个比喻，如果把主构造器看成人体的头部，那么 `init` 代码块就是颈部，次构造器就相当于身体其余部分。
...
之前我们讲了主构造器中的参数可以在属性中进行赋值，其实还可以在主构造器中声明属性：
...
不过，在实际开发中，还是推荐显式的将返回类型写出来，这样可以使代码阅读效率得到很大的提高。
...
我们知道 Kotlin 中函数没有返回值时，它的返回类型为 `Unit`。那么这里的函数 `sayHi` 的返回类型就是 `Unit`，只不过被我们省略了。所以，我们同样可以省略大括号并使用 `=` 符号进行连接：
...
当看到后面一长串的布尔值时，很难分清楚每个参数的用处，可读性很差。好在有了命名参数，我们可以这样写：
...
讲完了普通函数的简化写法，接下来看看 Kotlin 中字符串的方便写法。
...
这里字符串拼接的可读性差，写起来也比较麻烦，好在 Kotlin 还为我们提供了一种更加方便的方法：
...
我们在字符串中以 `$` 符号开头引用了一个变量，最终运行结果会将该变量的值合并到字符串中，这就是「字符串模板」。

字符串模板还可以使用 `$` 符号开头加上花括号将任意表达式括起来：
...
最终运行结果会将花括号中表达式的运算结果合并到字符串当中。

在字符串中，想使用表示字面值的 `$` 字符，可以这样写：
...
这里 `text` 中支持使用 `$` 符号插入变量，也可以进行换行操作，且末尾的 `\n` 转义字符不会生效，这就是「原生字符串」。输出结果如下：
...
至此，我们了解了 Kotlin 中字符串的那些更方便的写法，下面我们来看看数组与集合有哪些更方便的操作。
...
至此，我相信你对 `if/else` 与 `when` 相关的知识了解得差不多了，下面再来看看 `for` 循环控制有什么更简单的使用方法。
