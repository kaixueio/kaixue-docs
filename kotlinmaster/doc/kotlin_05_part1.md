...
「协程」源自 `Simula` 和 `Modula-2` 语言，前者是 19 世纪 60 年代在挪威计算机电子中心由 `奥利-约翰·达尔` 与 `克利斯登·奈加特` 所设计的，后者是由 1984 年的图灵奖得主 `尼克劳斯·埃米尔·维尔特` 在 1978 年设计发行。协程在 Kotlin 中使用 `coroutine` 代表协程，而 `coroutine` 这个术语早在 1958 年就被 `马尔文·康威` 发明并用于构建 `汇编程序`。

「协程」设计的初衷是为了让 `协作式多任务` 表示起来更加方便。所谓「协作式多任务」，和「线程」是有差别的。「线程」是操作系统层面的一个概念，表示能够进行运算调度的最小单元，没有包含任务的分发与合并处理。而「协作式多任务」是一个更具体的概念，它并没有直接和任何操作系统相关联，关心的是 `任务的分发`、 `分发的数量`、`子任务的合并` 等这些更明确的操作。

当在网上搜索「协程」时，我们会看到 Kotlin 官方文档上介绍“本质上，协程是轻量级的线程”，又或许是很多博客上所提到的”不需要从「用户态」切换到「内核态」“、”是「协作式」的“等等，作为「Kotlin 协程的初学者」，这些概念或许并不是那么容易让人理解。毕竟这些权威的解释往往是设计者经过长时间的总结提炼出来的，只看结果，而不管过程就不能理解协程。

其实我们学习 Kotlin 中的协程，一开始并不需要了解这么多概念。在 Kotlin 中，协程其实就是**一套由 Kotlin 官方所提供的线程控制 API**。就像 Java 中的 `Executor` 和 Android 中的 `AsyncTas`，Kotlin 中的协程也是一种对 Thread API 的封装，让我们可以在写代码时，不用关注多线程就能够很方便地写出并发操作。

在 Java 中要并发操作通常需要初始化并开启一个 Thread 👇
...
可以看到，相比 Java 可以少写很多，但和之前的 Java 代码却是完全等价的，摆脱不了直接使用线程的那些困难和不方便：

- 难以感知到线程什么时候执行结束
- 线程间的相互通行
- 多个线程的管理

为了解决以上的痛点，或许你会想到用「线程池」 👇
...
`AsyncTask` 是 Android 对线程池 `Executor` 的封装，相较于 `Executor`，它的 API 比较简单，上手更容易，适合写一些简单的并发逻辑，但缺点是无法指定线程的数量，使用起来也不如 `Executor` 那样灵活。看到这里你可能会想到功能更加强大，调用起来也更加方便的 `RxJava`，它也可以很方便地实现并发逻辑。但正如我们所说的，就是因为 `RxJava` 太强大了，如果我们的需求仅仅只是实现并发逻辑，用它反而显得不适合，只使用 `RxJava` 的并发功能那就有点大材小用了。

在 Android 开发中，我们所用到的多线程通常在最后是需要切回主线程，并将异步结果反馈到主线程上的。这种情况使用协程是最适合不过的 👇
...
「协程」、 `AsyncTask`、`Executor`、`RxJava` 都可以用来实现并发，那么「协程」相比于其他三个框架的优势是什么？

协程的好处，不是在于它提供了什么超越线程的能力，而是能够让我们所写的异步代码看起来和同步代码一样 👇
...
👆 使用这种写法， nameTv.text 的正确赋值是能够得到保证的，因为这里的 **getUser()** 是一个挂起函数，涉及到了 Kotlin 协程中最著名的「非阻塞式挂起」。这个名词看起来不是那么容易理解，别急，我们后续的文章会专门对这个概念进行讲解。我们现在先把这个概念放下，只需要记住协程就是像这样写的就行了，来看看协程好在哪。
...
前面的文章没有涉及到 `闭包` ，但其实它并不属于 Kotlin 中独有的概念，同样也不是什么新概念，在 「Java 8」中就已经支持。
